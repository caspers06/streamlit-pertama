...
st.title("\U0001F4C8 Aplikasi Siklus Akuntansi Perusahaan Dagang")
...
# Sidebar
st.sidebar.header("\U0001F4DD Input Transaksi")
...
st.subheader("5. \U0001F4CA Neraca Saldo Setelah Penyesuaian")
...
st.subheader("6. \U0001F4C3 Laporan Laba Rugi")
...
st.subheader("7. \U0001F501 Laporan Perubahan Modal")
...
st.subheader("8. \U0001F9FE Neraca")
...
st.subheader("9. \U0001F6AA Jurnal Penutup")
...
st.subheader("10. \U0001F4CC Neraca Saldo Setelah Penutupan")
...
st.subheader("\U0001F4C5 Ekspor Data Keuangan")
output = io.BytesIO()
with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
    ...
    df_modal.to_excel(writer, sheet_name="Perubahan Modal", index=False)
    ...
    for akun, df_akun in ledger_adj.items():
        sheet_name = akun[:31] if len(akun) > 31 else akun
        df_akun.to_excel(writer, sheet_name=f"Buku - {sheet_name}", index=False)

    writer.close()
    output.seek(0)

st.download_button(
    label="\U0001F4E4 Unduh Laporan Lengkap (Excel)",
    data=output,
